...
const Listbox = React.forwardRef<HTMLUListElement, any>((props2, ref) => (
  ...
  Listbox.displayName = 'Listbox';
    ...
    return (
      <Stack sx={{width: '100%', position: 'relative'}} direction="column" spacing={1} alignItems="center">
      <Autocomplete
      onChange={(event, newValue, reason) => {
        ...
        router.push(`/users/${newValue.name}`)
        } else {
          router.push(`/posts?filters=[{"field":"name", "operation":"==", "value":"${newValue.name}"}]`)
        }
        ...
        onOpen={async () => {
          ...
          const value =  search;
            ...
            if (value == search) {
              ...
              setResults([])
            fetch("../api/autocompleteUsers?advanced=" + props.advanced + "&search=" + search).then((data) => data.json()).then(res => {
              // wait 1 second
        ...
        onClose={() => {
          ...
          onInputChange={async(event, value)=> {
            ...
            setSearch(value)
              ...
              setResults([])
              fetch("../api/autocompleteUsers?advanced=" + props.advanced + "&search=" + value).then((data) => data.json()).then(res => {
                // wait 1 second
                // .map((option: any) => {
        //     const object = option
        //     object.type = "user"
